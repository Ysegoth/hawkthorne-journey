...
$lovedir = "bin\love-0.10.1-win64\"
$check = Test-Path "bin\love-0.10.1-win64\love.exe"
...
if($check -eq $false){
  ...
  $filename = (Get-Location).Path + "\bin\love-0.10.1-win64.zip"

  $check = Test-Path $filename

  if($check -eq $false){
    Write-Host "Downloading love2d..."
    $url = "https://bitbucket.org/rude/love/downloads/love-0.10.1-win64.zip"
...
if($args[0] -eq "run"){
  ...
  if($args.Length -ne 1){
    .\bin\love-0.10.1-win64\love.exe src $args[1..($args.Length-1)]
  }else{
    .\bin\love-0.10.1-win64\love.exe src
  }
}elseif($args[0] -eq "test"){
  Write-Host "Testing Journey to the Center of Hawkthorne..."
  .\bin\love-0.10.1-win64\love.exe src --test --console
}
